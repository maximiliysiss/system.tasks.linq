...
/// <c>OrderBy</c>, <c>OrderByDescending</c>, <c>ThenBy</c>, <c>ThenByDescending</c>, and <c>Reverse</c>.
/// </summary>
public static class OrderingExtensions
{
    ...
    public static async Task<IOrderedEnumerable<TSource>> OrderBy<TSource, TKey>(
        ...
        (await task.ConfigureAwait(false)).OrderBy(keySelector, comparer);
    ...
    public static async Task<IOrderedEnumerable<TSource>> OrderByDescending<TSource, TKey>(
        ...
        Func<TSource, TKey> keySelector,
        IComparer<TKey>? comparer = null) =>
        (await task.ConfigureAwait(false)).OrderByDescending(keySelector, comparer);

    /// <summary>
    /// Awaits the task and performs a subsequent ordering of the elements of the resulting sequence
    /// in ascending order according to a key, using an optional comparer.
    /// </summary>
    /// <typeparam name="TSource">The type of the elements in the source sequence.</typeparam>
    /// <typeparam name="TKey">The type of the key returned by <paramref name="keySelector"/>.</typeparam>
    /// <param name="task">A task whose result is an already-ordered sequence.</param>
    /// <param name="keySelector">A function to extract a key from each element.</param>
    /// <param name="comparer">A comparer to compare keys, or <see langword="null"/> to use the default comparer.</param>
    /// <returns>A task that represents an <see cref="IOrderedEnumerable{TSource}"/> whose elements are sorted by the secondary key in ascending order.</returns>
    public static async Task<IOrderedEnumerable<TSource>> ThenBy<TSource, TKey>(
        this Task<IOrderedEnumerable<TSource>> task,
        Func<TSource, TKey> keySelector,
        IComparer<TKey>? comparer = null) =>
        (await task.ConfigureAwait(false)).ThenBy(keySelector, comparer);

    /// <summary>
    /// Awaits the task and performs a subsequent ordering of the elements of the resulting sequence
    /// in descending order according to a key, using an optional comparer.
    /// </summary>
    /// <typeparam name="TSource">The type of the elements in the source sequence.</typeparam>
    /// <typeparam name="TKey">The type of the key returned by <paramref name="keySelector"/>.</typeparam>
    /// <param name="task">A task whose result is an already-ordered sequence.</param>
    /// <param name="keySelector">A function to extract a key from each element.</param>
    /// <param name="comparer">A comparer to compare keys, or <see langword="null"/> to use the default comparer.</param>
    /// <returns>A task that represents an <see cref="IOrderedEnumerable{TSource}"/> whose elements are sorted by the secondary key in descending order.</returns>
    public static async Task<IOrderedEnumerable<TSource>> ThenByDescending<TSource, TKey>(
        this Task<IOrderedEnumerable<TSource>> task,
        Func<TSource, TKey> keySelector,
        IComparer<TKey>? comparer = null) =>
        (await task.ConfigureAwait(false)).ThenByDescending(keySelector, comparer);
    ...
    public static async Task<IEnumerable<TSource>> Reverse<TSource>(this Task<IEnumerable<TSource>> task) =>
        (await task.ConfigureAwait(false)).Reverse();
}
